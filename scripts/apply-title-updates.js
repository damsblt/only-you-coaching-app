/**
 * Script pour mettre à jour les titres dans Neon selon le fichier de métadonnées
 * Met à jour automatiquement les correspondances haute confiance (score >= 90)
...
const JSON_FILE = path.join(__dirname, '..', 'temp', 'title-updates.json')
...
async function applyTitleUpdates() {
  try {
    ...
    if (!fs.existsSync(JSON_FILE)) {
      ...
      console.error('💡 Exécutez d\'abord: node scripts/update-titles-from-metadata.js')
      process.exit(1)
    }
    ...
    const reportFile = path.join(outputDir, 'title-updates-medium-confidence.txt')
    fs.writeFileSync(reportFile, report, 'utf8')
    
    // Sauvegarder les résultats
    const resultsFile = path.join(outputDir, 'title-updates-results.json')
