 ...
 * 
 * Prérequis:
 *   - Variables d'environnement configurées pour Supabase et Neon
 *   - Les tables doivent déjà exister dans Neon
 */
  ...
  try {
    ...
    for (const row of data) {
      try {
        ...
        // Construire la requête INSERT avec ON CONFLICT pour éviter les doublons
        const query = `
          INSERT INTO ${tableName} (${keys.join(', ')})
          VALUES (${placeholders})
          ON CONFLICT (id) DO UPDATE SET
          ${keys.filter(k => k !== 'id').map((k, i) => `${k} = $${i + 2}`).join(', ')}
        `
        
        await neonSql(query, [...values, ...values.filter((_, i) => keys[i] !== 'id')])
        inserted++
      } catch (insertError) {
...
async function verifyMigration(tableName) {
  try {
    const result = await neonSql(`SELECT COUNT(*) as count FROM ${tableName}`)
...
async function main() {
  console.log('🚀 Début de la migration Supabase → Neon\n')
