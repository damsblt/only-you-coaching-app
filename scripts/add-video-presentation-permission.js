...
/**
 * Script pour ajouter les permissions publiques pour video-presentation.mp4
 * et les fichiers vidéo à la racine du bucket S3
 */
...
async function main() {
  console.log('🔧 Ajout des permissions publiques pour video-presentation.mp4 dans la politique S3...\n')
  ...
  // 2. Vérifier si la règle pour video-presentation.mp4 existe déjà
  const videoPresentationRuleExists = currentPolicy.Statement.some(
    stmt => stmt.Sid === 'PublicReadRootVideos' || (
      stmt.Resource && (
        Array.isArray(stmt.Resource)
          ? stmt.Resource.some(r => r.includes('video-presentation.mp4') || r.includes('*.mp4'))
          : (typeof stmt.Resource === 'string' && (stmt.Resource.includes('video-presentation.mp4') || stmt.Resource.includes('*.mp4')))
      )
    )
  )

  if (videoPresentationRuleExists) {
    console.log('   ✅ La règle pour video-presentation.mp4 existe déjà dans la politique')
    ...
    console.log('   2. Assurez-vous que le fichier existe dans s3://' + bucketName + '/video-presentation.mp4')
    return
  }

  // 3. Ajouter la règle pour video-presentation.mp4 et les fichiers .mp4 à la racine
  console.log('2️⃣  Ajout de la règle pour video-presentation.mp4 et fichiers .mp4 à la racine...')
  const videoPresentationRule = {
    Sid: 'PublicReadRootVideos',
    Effect: 'Allow',
    Principal: '*',
    Action: 's3:GetObject',
    Resource: [
      `arn:aws:s3:::${bucketName}/video-presentation.mp4`,
      `arn:aws:s3:::${bucketName}/*.mp4`
    ]
  }

  currentPolicy.Statement.push(videoPresentationRule)
  ...
  try {
    ...
    console.log(`      https://${bucketName}.s3.${awsRegion}.amazonaws.com/video-presentation.mp4`)
    console.log('   3. Si toujours 403, vérifiez dans la console S3:')
    console.log('      - Permissions > Block public access (doit être désactivé)')
    console.log('      - Permissions > Bucket policy (doit contenir la règle PublicReadRootVideos)')
  } catch (error) {
