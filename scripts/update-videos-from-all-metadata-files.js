/**
 * Script pour mettre à jour les vidéos avec les métadonnées de tous les fichiers MD
 */
...
// Lire tous les fichiers MD du dossier
const files = await fs.readdir(metadataDir)
const mdFiles = files.filter(f => f.endsWith('.md') && f !== 'rapport-metadonnees.md')

console.log(`📂 Fichiers MD trouvés: ${mdFiles.length}`)
mdFiles.forEach(f => console.log(`   - ${f}`))

// Parser les métadonnées d'un fichier
function parseMetadata(content, filename) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line && !line.startsWith('-') && !line.startsWith('*') && !line.startsWith('#') && !line.includes(':') && !line.match(/^\d+\./)) {
      // Vérifier si la prochaine ligne contient "Muscle cible" ou si c'est un pattern connu
...
console.log('\n📖 Lecture et parsing de tous les fichiers MD...')
let allExercises = []

for (const file of mdFiles) {
  const filePath = path.join(metadataDir, file)
  const content = await fs.readFile(filePath, 'utf-8')
  const exercises = parseMetadata(content, file)
  allExercises = allExercises.concat(exercises)
  console.log(`   ✅ ${file}: ${exercises.length} exercices`)
}

console.log(`\n✅ Total: ${allExercises.length} exercices trouvés dans tous les fichiers`)
...
for (const video of videos) {
  ...
  try {
    ...
    updateDetails.push({
      ...
      oldDifficulty: video.difficulty,
      newDifficulty: updateData.difficulty
    })
    updatedCount++
  } catch (error) {
...
if (notFoundTitles.length > 0 && notFoundTitles.length <= 50) {
  console.log(`\n⚠️  Vidéos sans métadonnées (${notFoundTitles.length}):`)
